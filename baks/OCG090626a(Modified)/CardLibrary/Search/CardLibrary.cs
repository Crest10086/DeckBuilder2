        ...
        public static CardLibrary GetInstance()
        {
            if (instance == null)
            {
                ...
                if (KeepInMemory)
                {
                    ...
                    instance = new CardLibrary(Reader.Read(ramdir, null, null));
                    if (Directory.Exists(Path + "DIYCardIndex"))
                    {
                        ramdiydir = new Lucene.Net.Store.RAMDirectory(Path + "DIYCardIndex");
                        instance.AddDIYCards(Reader.Read(ramdiydir, null, null));
                    }
                }
                else
                {
                    CardsReader Reader = new LuceneReader();
                    instance = new CardLibrary(Reader.Read(Path + "CardIndex", null, null));
                    if (Directory.Exists(Path + "DIYCardIndex"))
                    {
                        instance.AddDIYCards(Reader.Read(Path + "DIYCardIndex", null, null));
                    }
                }
            }
            return instance;
